...
Structural admission failures reject new starts without aborting admitted descendants. SDK hosts can override every structural limit through `SubagentManagerOptions.delegationLimits`; tree-wide token, cost, and deadline budgets remain unlimited unless the host supplies finite values, and crossing a configured consumption budget cancels the whole tree. Per-child turn safeguards are configured separately.
