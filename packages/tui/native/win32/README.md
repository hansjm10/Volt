...
On Windows, the build uses the Microsoft C++ Build Tools from Visual Studio. `build.mjs` locates `VsDevCmd.bat`, initializes a developer environment for `x64` and `arm64`, and builds the addon with `cl.exe`/`link.exe`.
...
For non-Windows cross-builds, or for custom Windows toolchains, set `VOLT_TUI_WIN32_TOOLCHAIN=mingw` and provide MinGW-compatible compilers:

```sh
VOLT_TUI_WIN32_TOOLCHAIN=mingw \
