...
export interface TuiAltScreenOptions {
	/** Number of logical lines moved for each mouse-wheel event. Defaults to 3. */
...
export class TuiAltScreen extends TuiBase implements ViewportTUI {
	...
	private savedCapabilities: TerminalCapabilities | undefined;
	...
	private lastClick: ClickTarget | undefined;
	...
	private scrollbarDrag: ScrollbarDrag | undefined;
	private scrollbarHover: ScrollView | undefined;
	private activeSearch: ActiveSearch | undefined;
	private pressedUrl: string | undefined;
	...
	private parseWheelEvent(data: string): WheelEvent | undefined {
		...
		if (sgr) {
			const buttonValue = sgr[1];
			const xValue = sgr[2];
			const yValue = sgr[3];
			if (buttonValue === undefined || xValue === undefined || yValue === undefined) return undefined;
			const button = Number.parseInt(buttonValue, 10);
			if ((button & 64) === 0) return undefined;
			const direction = button & 3;
			if (direction !== 0 && direction !== 1) return undefined;
			return {
				direction: direction === 0 ? -1 : 1,
				x: Number.parseInt(xValue, 10) - 1,
				y: Number.parseInt(yValue, 10) - 1,
			};
		}
	...
	private parseSgrMouseEvent(data: string): SgrMouseEvent | undefined {
		...
		if (button === undefined || x === undefined || y === undefined || terminator === undefined) return undefined;
		return {
			button: Number.parseInt(button, 10),
			x: Number.parseInt(x, 10) - 1,
			y: Number.parseInt(y, 10) - 1,
