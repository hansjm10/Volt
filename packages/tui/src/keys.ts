...
function parseKittySequence(data: string): ParsedKittySequence | null {
	...
	if (csiUMatch) {
		...
		return {
			codepoint,
			...(shiftedKey === undefined ? {} : { shiftedKey }),
			...(baseLayoutKey === undefined ? {} : { baseLayoutKey }),
			modifier: modValue - 1,
			eventType,
		};
	}
...
function isWindowsTerminalSession(): boolean {
	const { WT_SESSION, SSH_CONNECTION, SSH_CLIENT, SSH_TTY } = process.env;
	return Boolean(WT_SESSION) && !SSH_CONNECTION && !SSH_CLIENT && !SSH_TTY;
}
