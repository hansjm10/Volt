...
describe("viewport layout", () => {
	...
	it("applies responsive visibility without leaving gaps", () => {
		const root = new VStack(
			[
				new Text("always", 0, 0),
				{ component: new Text("wide", 0, 0), visible: ({ width }) => width >= 20 },
				new Text("tail", 0, 0),
			],
			{ gap: 1 },
		);
		assert.deepStrictEqual(visibleLines(renderLayoutFrame(root, 10, 5, () => {}).lines), [
			"always",
			"",
			"tail",
			"",
		...
		assert.deepStrictEqual(visibleLines(renderLayoutFrame(root, 20, 5, () => {}).lines), [
			"always",
			"",
			"wide",
			"",
	...
	it("clips leaf content around the cursor marker", () => {
		...
		assert.ok(frame.lines[1]?.includes("\x1b_pi:c\x07"));
	});

	it("routes nested hits deepest-first while retaining the primary scroll view", () => {
		const inner = new ScrollView(new Text("inner 1\ninner 2\ninner 3", 0, 0), { overscroll: "contain" });
		const primary = new ScrollView(new VStack([{ component: inner, basis: 2 }, new Text("tail", 0, 0)]), {
			primary: true,
			overscroll: "chain",
		});
		const frame = renderLayoutFrame(primary, 10, 2, () => {});
		assert.deepStrictEqual(getScrollViewsAt(frame, 0, 0), [inner, primary]);
		assert.strictEqual(frame.primaryScrollView, primary);
		assert.strictEqual(inner.overscroll, "contain");
		assert.strictEqual(primary.overscroll, "chain");
		assert.strictEqual(inner.scrollBy(-1), -1);
	});

	it("updates the primary scroll view when pane focus changes", () => {
		const conversation = new ScrollView(new Text("conversation", 0, 0), { primary: true });
		const inspector = new ScrollView(new Text("inspector", 0, 0));
		const root = new HStack([conversation, inspector]);

		assert.strictEqual(renderLayoutFrame(root, 20, 2, () => {}).primaryScrollView, conversation);
		conversation.setPrimary(false);
		inspector.setPrimary(true);
		assert.strictEqual(renderLayoutFrame(root, 20, 2, () => {}).primaryScrollView, inspector);
	});

	it("keeps ANSI-safe horizontal composition within its allocation", () => {
		const frame = renderLayoutFrame(
			new HStack([
				{ component: new Text("\x1b[31mleft\x1b[0m", 0, 0), basis: 6 },
				{ component: new Text("界x", 0, 0), basis: 3 },
			]),
			9,
			1,
			() => {},
		);
		assert.strictEqual(stripTerminalSequences(frame.lines[0] ?? "").trimEnd(), "left  界x");
	});
});
