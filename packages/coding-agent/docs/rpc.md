...
- `list_review_workflows` returns `{ "workflows": […] }` with active workflows followed by a bounded window of retained terminal results. Each entry carries `workflowId`, `action`, `status` (`running`, `completed`, `cancelled`, or `failed`), a bounded `target` (`description`, `diffCommand`), optional `findingsCount`/`errorMessage`, and `startedAt`/`endedAt` timestamps. PR targets may include `target.pullRequest:{provider,number}` so clients can associate a running or durable review with Work without parsing display text. Durable PR targets may also include `target.context` metadata: `captureStatus`, captured and rendered issue/discussion counts, `renderedBytes`, `limitationCodes`, and `fingerprint`. The bounded reference never includes title, body, URL, refs, object ids, linked-issue text, or discussion text. Commit targets use the canonical object id; pull request descriptions use `PR #N` without the title or body. Clients reconnecting after missing `workflow_end` should list to discover finished reviews.
