...
- **Conversation authority:** SessionManager's generation-scoped authority to project and mutate the canonical conversation. Its status is either `available` or sticky `reconciliation_required`; the latter retains the first cause for which persisted state could not be proven to match that manager until a fresh manager reopens the session bytes.
...
1. **Definitive rollback:** the participant proves no delivery-coupled durable state committed and that the current manager still owns conversation authority. The attempt is `retained`; this is a transaction outcome, not a late revocation.
2. **Uncertain, irreversible, or authority-losing failure:** the participant cannot prove safe replay in the current generation. The logical delivery is `terminally_failed` for that Agent runtime generation. That generation consumes the delivery terminally and must not retry, requeue, or publish it as committed.

Atomic append failures classify two independent properties: storage effect (`not_started`, `rolled_back`, `uncertain`, or `committed`) and conversation authority (`available` or `reconciliation_required`). A non-committed effect is retainable only while authority remains available. An atomic replacement that reports failure is reconciled from exact bytes, not by attempting another replacement back to the preimage. Current-schema committed JSONL lines must be valid UTF-8 JSON objects; permissive legacy parsing cannot classify an atomic preimage. A malformed unterminated final fragment remains the supported torn-tail case, and a complete final object without its line delimiter remains committed. Strict preimage parsing and live-snapshot validation run inside the serialized per-file operation. Preimage, candidate, and post-sync visibility comparisons use exact bytes. If the disk state matches the manager's live snapshot and the exact expected preimage (including an originally missing file) remains visible after a failed write, the attempt is definitively rolled back with authority available. If the exact candidate is visible, SessionManager rolls forward: it fsyncs the target through a write-capable, non-truncating handle and then the supported parent directory, re-reads the target, and commits the staged indexes only if the exact candidate remains visible. Publication, direct RPC acceptance, and provider work remain behind that proof.

Malformed or unreadable committed state, unexpected file presence or absence, or a disk snapshot that differs from the manager's live snapshot prevents the write from starting but retires that manager's authority. If replacement visibility cannot be classified exactly, candidate synchronization fails, or post-sync bytes cannot be reverified, the persistence watermark remains rejected and authority is likewise retired. In either case conversation authority becomes sticky `reconciliation_required` with the first unresolved cause. The transition synchronously suspends the old projection source, discards conversation frames and canonical conversation-derived RPC responses that have not reached transport, retires correlated host interactions, and suppresses every later AgentSession event from that generation. Non-projection control and error responses remain available so the host can report the failure and request cleanup or runtime replacement. Staged indexes remain unpublished and every conversation, planning, transcript, queue, and client-input projection or mutation is unavailable rather than reporting either an old in-memory snapshot or unproven disk state. The only remaining capabilities are stable session identity and file path, abort, queue hand-back for runtime replacement, disposal/final drain, and runtime replacement itself. These capabilities must not begin extension hooks, MCP preparation, provider/helper completion, shell execution, queue mutation, planning transitions, custom-message mutation, or an Agent run.

A fresh SessionManager establishes a new authority generation by reopening the authoritative JSONL bytes. Replacement teardown may acknowledge the retired manager's recorded reconciliation failure even when validation stopped before a persistence watermark rejected; unrelated persistence, MCP, and subagent cleanup failures still fail replacement. Reopening the same file and session ID is a generation refresh, not a registry or lease rekey, but it still rotates projection authority and publishes a fresh cursor-zero bootstrap. An exact candidate contains the canonical delivery and completed client input, so startup completes it without replay. An authoritative non-candidate file may still contain the accepted queued input: startup may recover and replay that input exactly once in the fresh Agent generation because direct RPC acceptance, delivery publication, provider execution, and other post-proof side effects never crossed the failed durability boundary. This recovery does not retry the terminal delivery object from the failed generation.
...
Participant settlement therefore has a fail-fast join rule. Calling `AgentSession.dispose()` from the active upstream `settle(context)` callback still schedules the one real disposal operation, and ignoring its receipt is valid. Consuming that receipt through `await`, `then`, `catch`, or `finally` while the callback remains active rejects immediately instead of joining the Agent settlement that is waiting on the callback. The rule follows the callback across asynchronous hops, while a revocable callback scope removes participant authority when the callback settles so detached descendants cannot retain it. A captured receipt can be joined normally after leaving the callback, and unrelated external disposal callers continue to wait for participant settlement.

Participant implementations should migrate synchronous lifecycle intent to `AgentDeliveryTransactionContext.requestAbort("disposal")`, return their authoritative `committed`, `retained`, or `terminally_failed` outcome, let the Agent run settle, and only then await `AgentSession.dispose()` (or a previously captured disposal receipt) from external lifecycle code. `requestAbort()` records the abort source without awaiting the run that invoked the participant, so it is the safe reentrant capability.
...
#217 adds disk-backed reconciliation coverage in `packages/coding-agent/test/suite/regressions/217-uncertain-atomic-append-reconciliation.test.ts`. It proves exact-preimage retention, cross-platform exact-candidate roll-forward, proof-gated planning/RPC/provider publication, stale-manager retirement before writing, sticky reconciliation-required authority, immediate rejection before conversation side effects, queued canonical-response fencing, and cleanup/runtime replacement. It also proves that reopening a candidate completes without replay, while reopening accepted authoritative non-candidate state recovers exactly once in a fresh Agent generation with later queued input still ordered.
