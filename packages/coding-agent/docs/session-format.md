...
The first store open performs a one-time import of legacy `.jsonl` sessions found in that session directory. Successfully imported files move to `legacy-jsonl/`; failed inputs move to `legacy-jsonl-failed/` and are reported. The archives are not live session storage.
...
The exported header has `version: 5` for session entries and `snapshotVersion: 1` for the interchange envelope. Import also accepts older session versions and migrates them before committing to SQLite:

- **Version 1**: Linear entry sequence
- **Version 2**: Tree structure with `id`/`parentId` linking
- **Version 3**: Renamed `hookMessage` role to `custom`
- **Version 4**: Added stable commit ordinals
- **Version 5**: Replaced unreplayable legacy client-input WAL state

Canonical JSON admission does not require another session-format version.
...
A snapshot exported from a session with a persisted parent carries the host-local store locator needed to restore that relationship, never a legacy per-session file path:
...
Parse JSONL only when consuming an explicitly exported snapshot. Do not read `sessions.sqlite` directly or treat archived legacy files as current state.
