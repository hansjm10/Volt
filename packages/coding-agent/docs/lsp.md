...
The matching server must be installed on the exact inherited `PATH`. Volt does not implicitly execute `node_modules/.bin`. Bare commands are searched in PATH order; relative PATH entries are based at the canonical project workspace. On Windows, extensionless commands use `PATHEXT` order for PATH, project-relative, and absolute launch forms, while commands already ending in a configured executable extension are probed directly. Commands containing `/` or `\\` resolve from the project workspace. All remaining command entries are passed as literal argv through Volt's cross-platform spawn wrapper, without shell joining.
