...
| `workspaceManagement` | one mode required | Utility stream target. Payloads are `{ "purpose": "unregister_workspace" }`, on `worktrees.v1` hosts `{ "purpose": "manage_worktrees" }`, or on `agent_launch.v1` hosts `{ "purpose": "manage_agents" }`. |
...
`agent_launch.v1` is an optional host feature for cold-starting a configured daemon-owned agent on a `manage_agents` workspace-management stream. Clients first call `get_agent_launch_options`, then send its revision with `create_agent`. Launch IDs are idempotency keys scoped to the authenticated client and workspace. The host durably reserves a predetermined session ID, persists model/thinking/Fast/Plan policy, optionally creates a managed worktree, and publishes a detached runtime atomically. `kind:"created"` is returned only after runtime ownership, the client's durable last-session selection, and the final launch commit are all flushed; that commit is the final idempotency marker. Retries after the commit return `kind:"existing"`; reusing a launch ID with a different request returns `launch_conflict`.

The host revalidates the captured client grant, workspace registration/path, tool policy, and daemon admission throughout launch. A race that changes that authority before the final selection boundary returns `authorization_changed` and publishes no runtime. Catalog discovery exceptions are contained as correlated `success:false` RPC responses with `error:"agent_launch_catalog_unavailable"`; unexpected create-handler exceptions similarly use `agent_launch_unavailable` rather than terminating the management stream. If an incomplete launch created a worktree, cleanup always removes that worktree before deleting the durable receipt. A failed or thrown removal returns `cleanup_required`, retains the receipt for startup or explicit retry, and must be retried with the same launch request; the host never treats that response as a completed launch.
...
Workspace management streams with purpose `unregister_workspace` accept `unregister_workspace` and `list_workspace_directories`. The directory-listing RPC takes `workspaceName` plus optional relative `path`, and returns `directories:[{name,path}]` with relative paths only. Unregister refuses with `workspace_has_worktrees` while any persisted child worktree remains; clients must remove each worktree through `remove_worktree`, using `force:true` only as the user's explicit destructive choice. Management streams with purpose `manage_worktrees` (worktrees.v1) accept only `create_worktree`, `list_worktrees`, and `remove_worktree`. Management streams with purpose `manage_agents` (`agent_launch.v1`) accept only `get_agent_launch_options` and `create_agent`; successful creation durably updates last-session state, flushes the final launch commit, and retains the new daemon runtime detached. `authorization_changed` means authority changed during that transaction and the client must refresh/reconnect before retrying. `cleanup_required` means the same launch request is retryable only after the host can remove its launch-created worktree; the retained receipt remains the recovery marker. Any other valid RPC command receives `unsupported_on_workspace_management_stream`. Every management command must include a `workspaceName` matching the stream workspace (`session_mismatch` otherwise) and may not include extra fields (`invalid_request`); inbound host-local filesystem paths are always rejected.
...
The exact capability IDs are `conversation.observe.v1`, `conversation.control.v1`, `model.select.v1`, `integrations.manage.v1`, `worktrees.manage.v1`, `host.manage.v1`, `workspace.manage.v1`, and `diagnostics.upload.v1`. Command authorization is evaluated after the static remote command allowlist, which remains a hard ceiling. Capability denials use `error.code:"rpc_capability_denied"` and include `error.requiredCapability` where the response architecture supports structured errors. Session-only `set_model`/`set_thinking_level` (`persistDefault:false`) requires `model.select.v1`; persisting or omitting `persistDefault` additionally requires `host.manage.v1`. Cold launch discovery requires `model.select.v1`; creation requires `conversation.control.v1`, plus `model.select.v1` when model or thinking is explicit and `worktrees.manage.v1` when the request creates a worktree.
