...
Ready-plan feedback uses one staged transaction for direct prompts, steering, and follow-ups. Delivery preparation previews the draft state and checkpoint without mutating planning state. Synchronous delivery begin then transfers ownership, commits the ready-to-draft transition, rebuilds the Plan tool surface, and prefixes the checkpoint before the user message. Feedback revoked before begin leaves the ready plan and transcript unchanged; feedback observed after `delivery_start` is authoritative.
