...
`submit_plan` is blocked at the model tool boundary until the current runtime has observed a successful operation that resolved to a research-evidence capability (`workspace.read`, `network.read`, or `integration.read`). A resumed draft must therefore perform a fresh exploration call before submission. Direct SDK state-transition methods remain deterministic and do not synthesize tool evidence.
