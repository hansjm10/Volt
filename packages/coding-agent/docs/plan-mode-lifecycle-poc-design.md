...
Direct ready-plan feedback commits the return to `draft` and its canonical checkpoint before `before_agent_start`, so extensions inspect the target Plan tool surface and their per-turn system-prompt result is composed with the trusted Plan policy. Queued feedback cannot transition at enqueue time because it may still be cleared or fenced; the idempotent delivery-preparation hook performs the same transition only after that delivery is leased and prefixes its checkpoint before the user message. Agent request preparation then snapshots the already-synchronized runtime state without rebuilding the extension-provided prompt.

Preparation alone does not remove an admitted message. The active dispatcher tracks each entry as leased, emitting, committed, or revoked. A synchronous begin gate is evaluated from live lease state immediately before each delivery emits, so queue clearing can revoke untouched entries even after `nextAction` returns. Preparation or delivery failure restores every leased or emitting entry that never reached its commit event ahead of concurrently admitted work. Abort, disposal, queue clearing, and tree-generation fences retain their durable admission semantics without repurposing external client IDs.
