...
Inline images work in fullscreen terminals that support the Kitty graphics protocol, including Kitty and Ghostty. In iTerm2 they render as text placeholders because its inline-image protocol cannot delete or crop placements during application-owned scrolling. Regular mode continues to render iTerm2 inline images normally. See [Terminal setup](terminal-setup.md) for terminal-specific mouse, link, and paste behavior.
