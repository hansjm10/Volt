...
describe("regression #199: approved plan finalization", () => {
	...
	it("leaves revoked preparation without planning state or checkpoint side effects", async () => {
		...
		const run = harness.session.agent.continue();
	...
	it("transitions a ready plan after delivery admission and before the provider request", async () => {
	...
	it("does not automatically retry request preparation and keeps the admitted input canonical", async () => {
		const harness = await createHarness();
		harnesses.push(harness);
		await createReadyPlan(harness);
		harness.session.setSessionName("failed ready plan request preparation");
		...
		harness.session.agent.continue = async (options) => {
			...
			return await continueAgent(options);
		};

		await harness.session.prompt("Feedback whose planning transition fails");

		expect(automaticContinuations).toBe(0);
		expect(harness.session.agent.hasQueuedMessages()).toBe(false);
		expect(getUserTexts(harness)).toEqual(["Feedback whose planning transition fails"]);
		expect(
