...
<group n="6" title="Documentation, compatibility, and final validation">
  ...
  <item ref="F.2" status="resolved" prereq="E.2,F.1">
    ...
    <evidence>Resolved 2026-06-23: final validation passed with exact targeted host tests `node node_modules/vitest/dist/cli.js --run test/host-actions.test.ts test/rpc-transport-client.test.ts test/remote-iroh-core.test.ts test/suite/review.test.ts test/suite/agent-session-model-extension.test.ts` from `packages/coding-agent` (5 files, 143 tests), `npm run iroh:poc:test`, `npm run check`, `swift test` from `../volt-app/Packages/VoltClient` (23 XCTest tests plus 103 Swift Testing tests), XcodeBuildMCP `test_sim`, `xcodebuild -scheme Volt -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=27.0' test` from `../volt-app` (109 simulator tests), and `git diff --check -- .volt/ios-native-remote-ui-actions-design.md .volt/ios-native-remote-ui-actions-goal.md`. Manual iPhone 17 Pro iOS 27.0 simulator smoke on macOS 27.0 build 26A5353q used Demo transport `mock / local-demo`: connected from Settings, loaded Actions, invoked Review changes and verified Chat transcript output, toggled Fast mode to `Fast: low`, started a new conversation, switched to `Previous mock chat`, verified Actions refreshed after the session switch, and verified visible metadata stayed bounded to safe labels/short ids with no pairing secret, raw source path, prompt/skill body, raw model catalog, git diff, or tool metadata. Screenshots: `/tmp/volt-f2-demo-connected.png`, `/tmp/volt-f2-actions-loaded.png`, `/tmp/volt-f2-review-output.png`, `/tmp/volt-f2-fast-mode.png`, `/tmp/volt-f2-new-conversation.png`, `/tmp/volt-f2-session-switch.png`, `/tmp/volt-f2-actions-after-switch.png`. Live manual Iroh pairing and real-device smoke were not performed; Iroh behavior is covered by the scenario suite.</evidence>
  </item>
  ...
  <item ref="F.3" status="resolved" prereq="F.2" type="rollup">
    ...
    <evidence>Resolved 2026-06-23: all prior work-queue items A.1 through F.2 are resolved, final validation evidence is recorded, and the design rollup documents the v1 supported surface plus deferred work. Native remote UI actions are ready for iOS preview use with host-owned descriptors, safe Iroh discovery/invocation, shared built-in actions, Review/Fast mode cards, iOS command palette and Actions tab, descriptor-driven arguments/toggles, projected extension command groups, docs, and compatibility migration notes. Raw slash prompt compatibility remains supported for extension commands, skills, and prompt templates. Deferred future work remains explicit and not overclaimed: first-class `volt.registerAction()` cards, live manual Iroh/device smoke, remote model selection, package/profile settings, richer result payloads, and the shared preference descriptor layer for global defaults plus per-chat Fast mode/model overrides. V1 `thinking.fast_mode` remains session-local and non-persistent.</evidence>
  </item>
</group>
